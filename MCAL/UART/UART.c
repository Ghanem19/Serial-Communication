...
/**
 * @brief  UART_SendByte : Sends a string of char
 *
 * @param string : The first address of the printed string.
 * @return ERROR_STATUS : Indication to the function execution
 *
 **/
ERROR_STATUS UART_SendString(char* string)
{
	uint8 u8_index = 0;
	while(string[u8_index]){
		if(E_NOK == UART_SendByte(string[u8_index])){
			return E_NOK;
		}
		else{}
		u8_index++;
